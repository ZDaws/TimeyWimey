...
func storeEventArray(name: String, valArray: [Event])
{
    defaults.setValue(valArray, forKey: name)
    defaults.synchronize()
}

func retrieveEventArray(name: String) -> [Event] //must force unwrap when retrieving
{
    if let temp = defaults.valueForKey(name) as? [Event]
    {return temp}
    return invArray as! [Event]
}
